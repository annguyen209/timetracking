...
class Report {
	 ...
	 */
	public static function format_value( $p_key, $p_value, $csv=false ) {
		switch( $p_key ) {
			case 'user':
			...
			case 'issue':
				if($csv){
					$t_value = sprintf('%08d', $p_value) . ': ' . bug_get_field( $p_value, 'summary' );
				}else{
			...
			case 'bugnote_id':
				if($csv){
					$t_value = $p_value == 0 ? '' : $p_value;
				}else{
					$t_value = string_display_line( $p_value );
				}				
				break;
			default;
				if($csv && substr( $p_value, 0, 1 ) === "-"){
					$t_value = string_display_line( ' ' . $p_value );
				}else{
					$t_value = string_display_line( $p_value );
				}
