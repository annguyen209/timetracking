...
function user_can_edit_record_id( $p_record_id ) {
	...
	if( $t_record) {
		...
		$t_user_level = user_get_field( $t_user_id, 'access_level' );
		$t_can_edit = access_has_bug_level( plugin_config_get( 'edit_threshold' ), $t_record['bug_id'] );
		return $t_can_edit && ($t_record['user_id'] ==  $t_user_id || $t_user_level == 'ADMINISTRATOR' || $t_user_level == 'MANAGER') ;
	}
	return false;
}
...
function print_input_time_count( $p_time = null ) {
	...
	echo '<input type="text" placeholder="hh:mm" name="plugin_timetracking_time_input" class="form-control input-sm timetracking_time_input" value="' . $t_time_value . '" title="' . plugin_lang_get( 'time_input_tooltip' ) . '">';
...
function print_input_info( $p_info_text = null ) {
	echo '<textarea class="form-control input-sm" type="text" name="plugin_timetracking_info" value="' . $p_info_text . '">' . $p_info_text . '</textarea>';
}
...
function print_timetracking_inputs( $p_bug_id ) {
	...
	?>	
		<div class="table-responsive table-add-form">
			<table class="table table-bordered table-condensed table-striped">
				<tbody>
					<tr class="time_spent">
						<th><?php echo plugin_lang_get( 'time_spent' ) ?></th>
						<td>
							<div class="pull-left input-time">
					...
					<tr class="category">
						...
						<td><?php print_input_category() ?></td>
					</tr>
					<tr class="expenditure_date">
						<th ><?php echo plugin_lang_get( 'expenditure_date' ) ?></th>
						<td><?php print_input_date( 'plugin_timetracking_exp_date' ) ?></td>
					</tr>
					<tr class="information">
						<th ><?php echo plugin_lang_get( 'information' ) ?></th>
						<td><?php print_input_info() ?></td>
					</tr>
				</tbody>
			</table>
		</div>
...
function print_timetracking_category_option_list( $p_category = null ){
	foreach ( explode( PHP_EOL, plugin_config_get( 'categories' ) ) as $t_key ) {
		$t_option = '<option value="' . $t_key . '" ';
		if($p_category != null && $p_category == $t_key){
			$t_option = $t_option . ' selected ';
		}
		$t_option = $t_option .  '>' . string_display_line( $t_key ) . '</option>';
		echo $t_option;
